...
def _send_http_request(conn, http_method, uri, headers, body, send_buf_size):
    """
    Send an HTTP request using the supplied connection object and return response.
    
    Args:
        conn (httplib.HTTPConnection or urllib3.connectionpool.HTTPConnectionPool):
            The HTTP connection to use to make the request.
        ...
        headers (dict):
            A dictionary containing the headers to include in the request.
    
        body (None or file-like object or str):
            The message body content as a string or file like object representing the request payload.
            If None, no request payload will be sent.
    
        send_buf_size (int):
            The buffer size when sending request payload with `body`.
    
    Returns:
        requests.Response:
            The response returned by the HTTP server after processing the request.
